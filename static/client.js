...
document.addEventListener('DOMContentLoaded', () => {
        ...
        switch (type) {
            ...
            case 'new_message':
                ...
                if (payload.chatId === currentChatId) {
                    ...
                    // 如果不是当前聊天，显示小红点提示
                    const friendItem = document.querySelector(`.list-item[data-username="${payload.sender === username ? payload.chatId.replace(username, '').replace('-', '') : payload.sender}"]`);
    ...
    function addMessageToUI(msg) {
        // 关键修复：确保消息不会重复显示（多设备同步时，自己发的也会收到广播）
        ...
        const isScrolledToBottom = messagesContainer.scrollHeight - messagesContainer.clientHeight <= messagesContainer.scrollTop + 5; // 增加容错
        const item = document.createElement('div');
        ...
        if (isScrolledToBottom || msg.sender === username) {
    ...
    messageForm.onsubmit = (e) => {
        ...
        if (messageInput.value && currentChatId && ws && ws.readyState === WebSocket.OPEN) {
            ...
            const messagePayload = { chatId: currentChatId, content: messageInput.value, tempId: tempId };
            ws.send(JSON.stringify({ type: 'send_message', payload: messagePayload }));
            // 优化：本地立即显示的消息也用addMessageToUI，并带上临时ID
            addMessageToUI({ id: tempId, sender: username, content: messageInput.value, contentType: 'text' });
            messageInput.value = '';
        }
    };
    ...
    recallOption.onclick = () => {
        ...
        if (messageId && currentChatId && !messageId.startsWith('temp-')) {
