...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function handleServerMessage({ type, payload }) {
        ...
        switch (type) {
            ...
            case 'new_message':
                // 如果是自己的回声消息
                if (payload.sender === username && payload.isEcho) {
                    const tempMessage = document.querySelector(`[data-message-id^="temp-"]`);
                    // 如果能找到临时消息（说明是发送设备），就更新它
                    if (tempMessage) {
                        tempMessage.dataset.messageId = payload.id;
                    } else { // 否则（说明是其他设备），直接添加
    ...
    // 在UI上添加一条消息
    function addMessageToUI(msg) { const isScrolledToBottom = messagesContainer.scrollHeight - messagesContainer.clientHeight <= messagesContainer.scrollTop + 5; const item = document.createElement('div'); item.dataset.messageId = msg.id; if (msg.contentType === 'recalled') { item.className = 'message-recalled'; item.textContent = `${msg.sender} 撤回了一条消息`; } else { const isMine = msg.sender === username; item.className = isMine ? 'message mine' : 'message theirs'; item.innerHTML = `<div class="message-content"><div class="message-sender">${escapeHTML(msg.sender)}</div><div class="text">${escapeHTML(msg.content)}</div></div>`; if (isMine) { item.addEventListener('contextmenu', (e) => { e.preventDefault(); contextMenu.style.top = `${e.pageY}px`; contextMenu.style.left = `${e.pageX}px`; contextMenu.style.display = 'block'; contextMenu.dataset.targetMessageId = msg.id; }); } } messagesContainer.appendChild(item); if (isScrolledToBottom) { messagesContainer.scrollTop = messagesContainer.scrollHeight; } }
    // 切换聊天对象
